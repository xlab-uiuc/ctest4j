...
def gradle_clean_and_build_cmd(proj: str) -> str:
    return './gradlew clean build -x test' if proj == 'jmeter' else 'gradle clean build -x test'


def vanilla_gradle_cmd(proj: str) -> str:
    return './gradlew :src:core:test --no-rebuild -PchecksumIgnore --rerun-tasks' if proj == 'jmeter' else 'gradle test --no-rebuild --rerun-tasks'


def mapping_collection_gradle_cmd(proj: str) -> str:
    return './gradlew :src:core:test --no-rebuild -PchecksumIgnore --rerun-tasks -Pctest.config.save=true' if proj == 'jmeter' else 'gradle test --no-rebuild --rerun-tasks -Pctest.config.save=true'


def ctest_runner_gradle_cmd(proj: str) -> str:
    return './gradlew :src:core:test --no-rebuild -PchecksumIgnore --rerun-tasks -Pctest.mapping.dir=ctest/saved_mapping/' if proj == 'jmeter' else 'gradle test --no-rebuild --rerun-tasks -Pctest.mapping.dir=ctest/saved_mapping/'
